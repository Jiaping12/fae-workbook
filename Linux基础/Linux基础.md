...
bash ./*.sh
```
三种执行脚本的方式都是重新启动一个子shell，在子shell中执行此脚本，脚本中设置的变量在脚本执行完毕后不会保存。
```
source ./*.sh
. ./*.sh
```
两者等价，都是在当前shell进程中执行此脚本，而不是重新启动一个shell 在子shell进程中执行此脚本，并且脚本中设置的变量在脚本执行完毕后会保存下来。

source和bash执行脚本差别：source是在当前进程执行脚本，bash是增加新的进程执行脚本。
